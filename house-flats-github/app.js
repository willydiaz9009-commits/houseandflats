...
function whatsappUrl(item){return `https://wa.me/?text=${encodeURIComponent(`Hola House & Flats, me interesa ${item.title} en ${item.neighborhood}, ${item.city} por ${money(item)}/mes. ¿Está disponible?`)}`}
...
function makeIcon(item){
  ...
  return L.divIcon({className:'', html:`<div class="price-pin ${active}">${money(item)}</div>`, iconSize:[72,34], iconAnchor:[36,17]});
}
...
function findMatchingListings(lead){
  ...
  // Universo: misma ciudad; si no hay nada, mismo país.
  let pool = listings.filter(l => city && l.city === city);
  if(!pool.length && country) pool = listings.filter(l => l.country === country);
  ...
  if(!all.length){
    ...
    host.innerHTML = `
      <div class="wd-nomatch">
        ...
        <a class="wd-btn wd-btn-primary wd-btn-lg" target="_blank" rel="noopener" href="https://wa.me/?text=${encodeURIComponent(waText)}">💬 Hablar por WhatsApp</a>
      </div>`;
    return;
  }
...
function handleLeadSubmit(e){
  ...
  const recs = (matches.length ? matches : listings.map(item=>({...item,score:scoreLead(lead,item)})).sort((a,b)=>b.score-a.score)).slice(0,3);
  ...
  sendBtn.href = `https://wa.me/?text=${encodeURIComponent(sendMsg)}`;
